...
var path2, lsp6, import_vscode_languageserver_textdocument, import_vscode_uri5, WorkspaceShimService;
  ...
  "src/shims/workspace.ts"() {
    ...
    WorkspaceShimService = class extends Disposable {
      constructor(delegate, configurationShim, initWorkspaceFolders) {
        ...
        this._onDidOpenTextDocument = this._register(new lsp6.Emitter());
        this.onDidOpenTextDocument = this._onDidOpenTextDocument.event;
        this._onDidCloseTextDocument = this._register(new lsp6.Emitter());
        ...
        this._onDidRenameFiles = this._register(new lsp6.Emitter());
        ...
        this._onDidGrantWorkspaceTrust = this._register(new lsp6.Emitter());
        ...
        for (const f of initWorkspaceFolders ?? []) {
          ...
          const uri = import_vscode_uri5.URI.parse(f.uri);
          this._workspaceFolders.set(uri, {
            name: f.name || import_vscode_uri5.Utils.basename(uri),
            index: id,
            uri
          });
        }
      ...
      $getDocumentByLspUri(uri) {
        return this._documents.get(import_vscode_uri5.URI.parse(uri));
      }
      ...
      $openTextDocument(params) {
        ...
        const doc = import_vscode_languageserver_textdocument.TextDocument.create(uri, languageId, version, text);
        this._documents.set(import_vscode_uri5.URI.parse(uri), doc);
      ...
      $changeTextDocument(params) {
        ...
        const doc = this._documents.get(import_vscode_uri5.URI.parse(uri));
        if (!doc) {
          this.delegate.logMessage(lsp6.MessageType.Error, `File ${uri} not found`);
          return;
        }
        import_vscode_languageserver_textdocument.TextDocument.update(doc, changes, version);
      ...
      $closeTextDocument(params) {
        ...
        const vsUri = import_vscode_uri5.URI.parse(uri);
      ...
      $changeWorkspaceFolders(params) {
        ...
        event.added.forEach((folder) => {
          const uri = import_vscode_uri5.URI.parse(folder.uri);
          const addFolder = {
            name: folder.name,
      ...
      $renameFiles(params) {
        const renamedFiles = params.files.map(({ newUri, oldUri }) => ({
          newUri: import_vscode_uri5.URI.parse(newUri),
          oldUri: import_vscode_uri5.URI.parse(oldUri)
        }));
      ...
      asRelativePath(pathOrUri, includeWorkspace) {
        const uri = import_vscode_uri5.URI.isUri(pathOrUri) ? pathOrUri : import_vscode_uri5.URI.file(pathOrUri);
      ...
      async openTextDocument(nameOrUri) {
        const uri = typeof nameOrUri === "string" ? import_vscode_uri5.URI.file(nameOrUri) : nameOrUri;
        ...
        await pending.wait();
        const doc = this._documents.get(uri) ?? import_vscode_languageserver_textdocument.TextDocument.create(uri.toString(), "unknown", 0, "");
...
var l10n, os2, import_vscode_languageserver_protocol4, import_vscode_uri6, import_vscode_languageserver_protocol5, extensions, languages, commands, window, env, workspace, chat, EventEmitter, Uri;
var init_shims = __esm({
  "src/shims/index.ts"() {
    ...
    Uri = new Proxy(import_vscode_uri6.URI, {
      get(target, p) {
        return target[p] ?? import_vscode_uri6.Utils[p];
      }
    });
  }
});
...
function isSupportedLanguageMode(doc) {
  return languages.match([typescript, typescriptreact, javascript, javascriptreact], doc) > 0;
}
      ...
      getQuoteStylePreference(config3) {
        switch (config3.get("quoteStyle")) {
          ...
          case "double":
            return "double";
          default:
            return this.client.apiVersion.gte(API.v333) ? "auto" : void 0;
        }
      }
...
var Range3, Position3, Location2, TextEdit2, WorkspaceEdit2, SymbolKind2, CompletionTriggerKind3, OrganizeImportsMode2;
var init_typeConverters = __esm({
  "src/typescript-language-features/src/typeConverters.ts"() {
    ...
    init_protocol_const();
    ((Range4) => {
      Range4.fromTextSpan = (span) => (0, Range4.fromLocations)(span.start, span.end);
      Range4.toTextSpan = (range) => ({
        start: Position3.toLocation(range.start),
        end: Position3.toLocation(range.end)
      });
      Range4.fromLocations = (start, end) => new Range(
        Math.max(0, start.line - 1),
        Math.max(start.offset - 1, 0),
        Math.max(0, end.line - 1),
        Math.max(0, end.offset - 1)
      );
      Range4.toFileRange = (range) => ({
        startLine: range.start.line + 1,
        startOffset: range.start.character + 1,
        endLine: range.end.line + 1,
        endOffset: range.end.character + 1
      });
      Range4.toFileRangeRequestArgs = (file2, range) => ({
        file: file2,
        ...(0, Range4.toFileRange)(range)
      });
      Range4.toFileRangesRequestArgs = (file2, ranges) => ({
        file: file2,
        ranges: ranges.map(Range4.toFileRange)
      });
      Range4.toFormattingRequestArgs = (file2, range) => ({
        file: file2,
    ...
    ((Location3) => {
      Location3.fromTextSpan = (resource, tsTextSpan) => new Location(resource, Range3.fromTextSpan(tsTextSpan));
    })(Location2 || (Location2 = {}));
    ((TextEdit3) => {
      TextEdit3.fromCodeEdit = (edit) => new TextEdit(
        Range3.fromTextSpan(edit),
        edit.newText
      );
    })(TextEdit2 || (TextEdit2 = {}));
    ((WorkspaceEdit3) => {
        ...
        for (const edit of edits) {
          ...
          for (const textChange of edit.textChanges) {
            workspaceEdit.replace(
              resource,
              Range3.fromTextSpan(textChange),
              textChange.newText
            );
          }
        }
...
function fromProtocolCallHierarchyItem(item) {
  ...
  const result = new CallHierarchyItem(
    ...
    detail,
    Uri.file(item.file),
    Range3.fromTextSpan(item.span),
    Range3.fromTextSpan(item.selectionSpan)
  );
...
function fromProtocolCallHierarchyIncomingCall(item) {
  return new CallHierarchyIncomingCall(
    ...
    item.fromSpans.map(Range3.fromTextSpan)
  );
}
      ...
      async resolveCodeLens(codeLens, token) {
        ...
        const locations = response.body.map((reference) => (
          ...
          new Location(
            ...
            reference.start.line === reference.end.line ? Range3.fromTextSpan(reference) : new Range(
            ...
            if (text) {
              if (/^https?:/.test(text)) {
                ...
                if (parts2.length === 1) {
                  ...
                  const linkText = parts2.slice(1).join(" ");
...
var init_completions = __esm({
  "src/typescript-language-features/src/languageFeatures/completions.ts"() {
    ...
    MyCompletionItem = class _MyCompletionItem extends CompletionItem {
      constructor(position, document2, tsEntry, completionContext, metadata, client) {
        ...
        this.commitCharacters = _MyCompletionItem.getCommitCharacters(completionContext, tsEntry);
      ...
      getRangeFromReplacementSpan(tsEntry, completionContext) {
        ...
        let replaceRange = Range3.fromTextSpan(tsEntry.replacementSpan);
    ...
    TypeScriptCompletionItemProvider = class {
      ...
      async provideCompletionItems(document2, position, token, context) {
        ...
        if (this.client.apiVersion.gte(API.v300)) {
          ...
          isIncomplete = !!response.body.isIncomplete || ((_a = response.metadata) == null ? void 0 : _a.isIncomplete);
          entries = response.body.entries;
          metadata = response.metadata;
          if (response.body.optionalReplacementSpan) {
            optionalReplacementRange = Range3.fromTextSpan(response.body.optionalReplacementSpan);
          }
        } else {
          const response2 = await this.client.interruptGetErr(() => this.client.execute("completions", args, token));
          if (response2.type !== "response" || !response2.body) {
            return void 0;
          }
          entries = response2.body;
        ...
        for (const entry of entries) {
          if (!shouldExcludeCompletionEntry(entry, completionConfiguration)) {
            const item = new MyCompletionItem(position, document2, entry, completionContext, metadata, this.client);
            item.command = {
  ...
  "src/typescript-language-features/src/languageFeatures/copyPaste.ts"() {
    ...
    _DocumentPasteProvider = class _DocumentPasteProvider {
      ...
      async provideDocumentPasteEdits(document2, ranges, dataTransfer, _context, token) {
        ...
        let copiedFrom;
        if (metadata) {
          const spans = metadata.ranges.map(Range3.toTextSpan);
        ...
        const response = await this._client.interruptGetErr(() => this._client.execute("getPasteEdits", {
          ...
          pastedText: [text],
          pasteLocations: ranges.map(Range3.toTextSpan),
          copiedFrom
        }, token));
        ...
        for (const edit2 of response.body.edits) {
          additionalEdit.set(this._client.toResource(edit2.fileName), edit2.textChanges.map(TextEdit2.fromCodeEdit));
        }
      ...
      static convertNavTree(resource, output, item) {
        ...
        for (const span of item.spans) {
          ...
          for (const child of children) {
            if (child.spans.some((span2) => !!range.intersection(Range3.fromTextSpan(span2)))) {
  ...
  "src/typescript-language-features/src/languageFeatures/formatting.ts"() {
    ...
    TypeScriptFormattingProvider = class {
      ...
      async provideDocumentRangeFormattingEdits(document2, range, options, token) {
        ...
        const args = Range3.toFormattingRequestArgs(file2, range);
        const response = await this.client.execute("format", args, token);
        if (response.type !== "response" || !response.body) {
          return void 0;
        }
        return response.body.map(TextEdit2.fromCodeEdit);
      }
    ...
    TypeScriptHoverProvider = class {
      ...
      async provideHover(document2, position, token) {
        ...
        return new Hover(
          ...
          Range3.fromTextSpan(response.body)
        );
      }
  ...
  "src/typescript-language-features/src/languageFeatures/linkedEditing.ts"() {
    ...
    LinkedEditingSupport = class {
      ...
      async provideLinkedEditingRanges(document2, position, token) {
        ...
        return new LinkedEditingRanges(response.body.ranges.map((range) => Range3.fromTextSpan(range)), wordPattern);
      }
    };
          ...
          mapping: {
            ...
            focusLocations: context.documents.map((documents) => {
              return documents.flatMap((contextItem) => {
                ...
                return contextItem.ranges.map((range) => ({ file: file3, ...Range3.toTextSpan(range) }));
              });
            })
          }
        }, token);
...
function findScopeEndLineFromNavTreeWorker(startLine, navigationTree) {
  for (const node of navigationTree) {
    const range = Range3.fromTextSpan(node.spans[0]);
...
var init_refactor = __esm({
  "src/typescript-language-features/src/languageFeatures/refactor.ts"() {
    ...
    init_shims();
    import_vscode_uri8 = require("vscode-uri");
    init_learnMoreAboutRefactorings();
    init_fileSchemes();
    init_schemes();
    init_api2();
    ...
    _MoveToFileRefactorCommand = class _MoveToFileRefactorCommand {
      ...
      async execute(args) {
        ...
        const fileSuggestionArgs = {
          ...Range3.toFileRangeRequestArgs(file2, args.range),
          action: "Move to file",
      ...
      async getTargetFile(document2, file2, range) {
        ...
        const updateItems = () => {
          ...
          const destinationItems = body.files.map((file3) => {
            ...
            const parentDir = import_vscode_uri8.Utils.dirname(uri);
            const filename = import_vscode_uri8.Utils.basename(uri);
            let description2;
            if (workspaceFolder) {
            ...
            return {
              file: file3,
              label: import_vscode_uri8.Utils.basename(uri),
        ...
        if (picked === selectExistingFileItem) {
          const picked2 = await window.showOpenDialog({
            ...
            defaultUri: import_vscode_uri8.Utils.dirname(document2.uri)
          });
    ...
    InlinedCodeAction = class _InlinedCodeAction extends CodeAction {
      ...
      async resolve(token) {
        ...
        const args = {
          ...Range3.toFileRangeRequestArgs(file2, this.range),
    ...
    _TypeScriptRefactorProvider = class _TypeScriptRefactorProvider {
      ...
      static isOnSignatureName(node, range) {
        ...
        if (this._declarationKinds.has(node.kind)) {
          if (node.nameSpan) {
            const convertedSpan = Range3.fromTextSpan(node.nameSpan);
            if (range.intersection(convertedSpan)) {
              return true;
            }
          }
          if (!node.nameSpan && /\bexport\b/.test(node.kindModifiers) && node.spans.length) {
            const convertedSpan = Range3.fromTextSpan(node.spans[0]);
      ...
      async provideCodeActions(document2, rangeOrSelection, context, token) {
        ...
        const response = await this.interruptGetErrIfNeeded(context, () => {
          ...
          const args = {
            ...Range3.toFileRangeRequestArgs(file2, rangeOrSelection),
      ...
      async prepareRename(document2, position, token) {
        ...
        switch (response.type) {
          case "rename": {
            ...
            return Range3.fromTextSpan(renameInfo.triggerSpan);
          }
          case "jsxLinkedEditing": {
            return response.spans.map(Range3.fromTextSpan).find((range) => range.contains(position));
          }
        }
      }
...
var _SmartSelection, SmartSelection;
var init_smartSelect = __esm({
  "src/typescript-language-features/src/languageFeatures/smartSelect.ts"() {
    ...
    init_typeConverters();
    init_dependentRegistration();
    _SmartSelection = class _SmartSelection {
      constructor(client) {
        this.client = client;
      }
  ...
  "src/typescript-language-features/src/configuration/configuration.ts"() {
    ...
    BaseServiceConfigurationProvider = class {
      ...
      readUseVsCodeWatcher(configuration) {
        return configuration.get("typescript.tsserver.experimental.useVsCodeWatcher", false);
      }
      ...
      readWebTypeAcquisition(configuration) {
        return configuration.get("typescript.tsserver.web.typeAcquisition.enabled", false);
      }
          ...
          if (this.areProjectDiagnosticsEnabled()) {
            ...
            if (this.areRegionDiagnosticsEnabled()) {
              requestFiles = coalesce(fileEntries.map((entry) => {
                ...
                if (file2 && ranges) {
                  return Range3.toFileRangesRequestArgs(file2, ranges);
                }
                return file2;
              }));
            } else {
  ...
  "src/typescript-language-features/src/tsServer/spawner.ts"() {
    ...
    _TypeScriptServerSpawner = class _TypeScriptServerSpawner {
      ...
      getCompositeServerType(version, capabilities, configuration) {
        ...
        switch (configuration.useSyntaxServer) {
          ...
          case 0 /* Never */:
            return 0 /* Single */;
          case 2 /* Auto */:
            if ((_a = version.apiVersion) == null ? void 0 : _a.gte(API.v340)) {
              return ((_b = version.apiVersion) == null ? void 0 : _b.gte(API.v400)) ? 2 /* DynamicSeparateSyntax */ : 1 /* SeparateSyntax */;
            }
            return 0 /* Single */;
        }
      }
      ...
      getTsServerArgs(kind, configuration, currentVersion, apiVersion, pluginManager, cancellationPipeName) {
        ...
        if (apiVersion.gte(API.v544) && configuration.useVsCodeWatcher && !apiVersion.isYarnPnp()) {
...
var path9, ServerState, emptyAuthority, inMemoryResourcePrefix, TypeScriptServiceClient, ServerInitializingIndicator;
  ...
  "src/typescript-language-features/src/typescriptServiceClient.ts"() {
    ...
    TypeScriptServiceClient = class extends Disposable {
      ...
      startService(resendModels = false) {
        ...
        handle.onExit((data) => {
          ...
          this.logTelemetry("tsserver.exitWithCode", { code: code ?? void 0, signal: signal ?? void 0 });
          if (this.token !== mytoken) {
            return;
          }
          if (((_a = handle.tsServerLog) == null ? void 0 : _a.type) === "file") {
      ...
      createFileSystemWatcher(id, pattern, ignoreChangeEvents) {
        ...
        const watcher = disposable.add(workspace.createFileSystemWatcher(pattern, { excludes: [], ignoreChangeEvents }));
    ...
    ServerInitializingIndicator = class extends Disposable {
      ...
      startedLoadingProject(projectName) {
        ...
        if (((_a = this._task) == null ? void 0 : _a.project) === projectName) {
          ...
          window.withProgress({
            location: 10 /* Window */,
            title: l10n.t("Initializing JS/TS language features")
          }, () => pending);
        }
      }
      createPending() {
        let resolve = null;
        const pending = new Promise((resolv) => resolve = resolv);
...
var init_updatePathsOnRename = __esm({
  "src/typescript-language-features/src/languageFeatures/updatePathsOnRename.ts"() {
    ...
    UpdateImportsOnFileRenameHandler.minVersion = API.v300;
  }
});
      ...
      async diagnosticsReceived(kind, resource, diagnostics, spans) {
        ...
        if (language) {
          language.diagnosticsReceived(
            ...
            spans == null ? void 0 : spans.map((span) => Range3.fromTextSpan(span))
          );
        }
      }
      configFileDiagnosticsReceived(event) {
        ...
        this.findLanguage(this.client.toResource(body.configFile)).then((language) => {
          language == null ? void 0 : language.configFileDiagnosticsReceived(this.client.toResource(body.configFile), body.diagnostics.map((tsDiag) => {
            const range = tsDiag.start && tsDiag.end ? Range3.fromTextSpan(tsDiag) : new Range(0, 0, 0, 1);
...
var import_vscode_uri9 = require("vscode-uri");
init_share();
init_shims();
init_types2();
init_barrier();
...
init_cjs_shims();
init_shims();
var lsp7 = __toESM(require("vscode-languageserver-protocol"));
...
function getWordAtText(column, text, textOffset = 0) {
  if (text.length > config2.maxLen) {
    ...
    text = text.substring(start, column + config2.maxLen / 2);
...
var TSLspConverter = class extends LspInvariantConverter {
  constructor(clientCapabilities) {
        ...
        if (entry._type === 1 /* File */) {
          ...
          if (!entry.from) {
            if (
              ...
              !((_c = entry.options) == null ? void 0 : _c.ignoreIfExists) && !resouceOpKinds.includes(lsp7.ResourceOperationKind.Create)
            ) {
            ...
            if (!resouceOpKinds.includes(lsp7.ResourceOperationKind.Rename)) {
            ...
            if (!resouceOpKinds.includes(lsp7.ResourceOperationKind.Delete)) {
      ...
      if (hasResourceOp) {
        return {
          documentChanges: docChanges.map((d) => {
            if (!import_vscode_uri7.URI.isUri(d)) {
              return d;
            } else {
              return {
    ...
    this.convertTextDocumentFromLsp = (textDocument) => {
      const uri = import_vscode_uri7.URI.parse(textDocument.uri);
      const doc = {
        uri,
        get version() {
        ...
        getWordRangeAtPosition(position) {
          const line = this.getText(
            new Range(position.line, 0, position.line, Number.MAX_VALUE)
          );
        ...
        lineAt(lineOrPosition) {
          ...
          const lineText = textDocument.getText(
            lsp7.Range.create(
              lsp7.Position.create(line, 0),
              lsp7.Position.create(line, Number.MAX_VALUE)
            )
          );
          return {
            lineNumber: line,
    ...
    this.convertCompletionItem = (item, data) => {
      ...
      return {
        ...
        insertTextMode: item.keepWhitespace === void 0 ? void 0 : item.keepWhitespace ? lsp7.InsertTextMode.adjustIndentation : lsp7.InsertTextMode.asIs,
        insertTextFormat: isSnippet ? lsp7.InsertTextFormat.Snippet : lsp7.InsertTextFormat.PlainText,
        insertText,
        textEdit,
      ...
      if (diagnostic.relatedInformation) {
        d.relatedInformation = diagnostic.relatedInformation.map((r) => {
          const location = new Location(
            import_vscode_uri7.URI.parse(r.location.uri),
    ...
    this.convertHover = (hover) => {
      ...
      for (const content of hover.contents) {
        if (lsp7.MarkedString.is(content)) {
    ...
    this.convertCallHierarcgyItemFromLsp = (item) => {
      return new CallHierarchyItem(
        ...
        item.name,
        item.detail ?? "",
        import_vscode_uri7.URI.parse(item.uri),
        Range.of(item.range),
    ...
    this.convertFoldingRange = (range) => {
      ...
      switch (range.kind) {
        case 1 /* Comment */:
          kind = lsp7.FoldingRangeKind.Comment;
          break;
        case 3 /* Region */:
          kind = lsp7.FoldingRangeKind.Region;
          break;
        case 2 /* Imports */:
          kind = lsp7.FoldingRangeKind.Imports;
          break;
        default:
          break;
      }
...
var TSCodeActionFeature = class extends Disposable {
  ...
  async codeAction(doc, params, token) {
    ...
    const baseVscCtx = {
      ...
      triggerKind: ctx.triggerKind ?? lsp8.CodeActionTriggerKind.Invoked
    };
    const results = [];
  ...
  constructor(commands3) {
    ...
    this._register(
      commands3.registerCommand(_CompletionCache.id, (...args) => {
        ...
        if (!item) {
          throw new lsp9.ResponseError(lsp9.ErrorCodes.InvalidParams, "completion item data missing");
        }
...
var TSCompletionFeature = class extends Disposable {
  ...
  async completion(doc, params, token) {
    ...
    let results = [];
    let isIncomplete = false;
    if (((_a = params.context) == null ? void 0 : _a.triggerKind) === lsp9.CompletionTriggerKind.TriggerForIncompleteCompletions && this.lastCompleteResult) {
    ...
    return lsp9.CompletionList.create(resultItems, isIncomplete);
  }
...
function createTSLanguageServiceDelegate(converter) {
  ...
  function onHandler(event) {
    return (handler) => {
      ...
      return lsp10.Disposable.create(() => {
...
function createTSLanguageService(initOptions) {
  ...
  function wrapRequestHandler(handler) {
    return async (params, token = lsp11.CancellationToken.None) => {
  ...
  const tsLanguageService = {
    ...
    codeLensResolve: wrapRequestHandler(async (item, token) => {
      ...
      const refLens = new ReferencesCodeLens4(
        import_vscode_uri9.URI.parse(item.data.document),
        item.data.file,
